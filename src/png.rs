...
impl Png {
    ...
    pub fn from_file(path: &str) -> Result<Png, Box<dyn std::error::Error>> {
        let file = std::fs::File::open(path)?;
        let mut reader = std::io::BufReader::new(file); 
        let png = <Png as TryFrom<&mut dyn std::io::Read>>::try_from(&mut reader)?;
        Ok(png)
    }

    pub fn to_file(&self, path: &str) -> Result<u64, std::io::Error> {
        ...
        if index > 0 {
            size += stream.write(&buf[..=index])? as u64;
        }
        stream.flush()?;
        stream.get_mut().set_len(size)?;
        Ok(size)
    }
    ...
    pub fn upinsert(&mut self, chunk_type: &str, data: Vec<u8>) -> Result<(), Error>{
        for chunk in self.chunks.iter_mut() {
            if chunk.chunk_type() == chunk_type {
                chunk.update_data(data);
                return Ok(());
            }
        }
	...
	pub fn chunk_by_type(&self, name: &str) -> Option<&Chunk> {
		self.chunks.iter().find(|chunk: &&Chunk| chunk.chunk_type() == name)
	}

    #[allow(dead_code)]
	pub fn remove_chunk(&mut self, name: &str) -> Result<(), Error>{
		for (index, chunk) in self.chunks.iter().enumerate() {
			if chunk.chunk_type() == name {
    ...
    pub fn iter(& self) -> impl Iterator<Item = u8> + '_ {
        let flat_map = self.chunks.iter().flat_map(
...
impl TryFrom<&[u8]> for Png 
{
	...
	fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        <Png as TryFrom<&mut dyn Read>>::try_from(&mut value)
	}
}

impl TryFrom<&mut dyn Read > for Png 
{
    type Error = Error;
	fn try_from(value: &mut dyn Read) -> Result<Self, Self::Error> {
        let mut buffer = [0u8;8];
		value.read_exact(&mut buffer).map_err(|x| Error::HeaderReadError(x))?;
		if buffer.len() != Png::STANDARD_HEADER.len() {
			Err(Error::HeaderLenMisMatch { actual: buffer.len() })
		} else if buffer != Png::STANDARD_HEADER {
			Err(Error::HeaderMisMatch { bytes: buffer.to_vec() })
		} else {
			let mut size_buffer = [0u8;4];
			let mut chunks: Vec<Chunk> = Vec::new();

			loop {
                let read_result = value.read_exact(&mut size_buffer)
					.map_err(|e| Error::ChunkSizeReadError(e) );
				if let Err(Error::ChunkSizeReadError(e)) = read_result {
                    if e.kind() == std::io::ErrorKind::UnexpectedEof {
                        break 
                    };
                }
				let size = u32::from_be_bytes(size_buffer) as usize;
				let mut data_buffer: Vec<u8> = vec![0u8;4 + size + 4];
				value.read_exact(&mut data_buffer)
					.map_err(|e| Error::ChunkDataReadError(e))?;
				let chunk = Chunk::try_from((size, &data_buffer))?;
				chunks.push(chunk);
			}
